...
async def new_text_callback(reply_message,text: str):
...
async def on_message(message: discord.Message) -> None:
    ...
    if message.author == bot.user or message.author.bot:
        return
    if message.content[0:4] == "!ai " and message.channel.id == 1094117653142048830:
        inp = ''.join([i if ord(i) < 128 else ' ' for i in message.content[4:]])
        #text = 'Your prompt is "'+text+'" And your response is:'
        print("Prompting AI with: '"+inp+"'")
        reply_message = await message.reply(": ")
        generated_text = await model.generate(inp, n_predict=384, new_text_callback=lambda x: asyncio.ensure_future(new_text_callback(reply_message, x)), n_threads=8, n_batch=128)
